...
include 'connection.php';
    ...
    while($row = mysqli_fetch_assoc($result)) {
        $name = $row["lab_assistant_name"];
		$img = $row["image"];
        $qr = $row["qr_path"];
		$dob = $row["DOB"];
        $gender = $row["Gender"];$street = $row["address"];
		$city = $row["city"];
		$dist = $row["district"];
		$state = $row["state"];
		$pin = $row["pincode"];
	...
	imagepng($front,'img/lab/card/front_'.$id.'.png');
	imagedestroy($front);
	imagepng($back,'img/lab/card/back_'.$id.'.png');
	imagedestroy($back);    
	header("location:lab_send_card.php");
?>