    ...
    <div id="app">
      <div class="sidebar">
        ...
        <section class="scans">
          ...
          <transition-group name="scans" tag="ul">
            <li v-for="scan in scans" :key="scan.date" :title="scan.content"><a href="http://localhost/bootstrap2/lab_login.php"> {{scan.content}} </a></li>
      ...
      <transition-group name="scans" tag="ul">
          <li style="color: yellow;" v-for="scan in scans" :key="scan.date" :title="scan.content">{{ scan.content }}
            <form action="http://localhost/preclinic/lab_login.php" method="post">
              <input style="color: yellow;"type="hidden" name="qr1" :key="scan.date" :value="scan.content" />
